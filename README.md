...
| @openliulan/stealth | 1.0.0 |
| @openliulan/viz | 1.0.0 |
...
| M9 | **防检测模块**：反指纹注入、自动化控制隐藏、人类行为模拟（`@openliulan/stealth`，12 项测试通过） | ✅ |
| M10 | **条件触发式可视化**：按需启用、AI 决策透明化、用户确认回环（`@openliulan/viz`，11 项测试通过） | ✅ |
...
| 可视化 Web 面板 | 条件触发式可视化升级为 Web GUI（当前为结构化/终端渲染，后续加浏览器面板） | v1.2 |
| **防检测增强** | Stealth 模块集成 camoufox/anti-detect 浏览器、WebRTC IP 泄漏防护、更精细的指纹控制 | v1.2 |
